...
function ProjectToGuide() {
    ...
    const getData = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/t/guideProjects/${userID}`
        );
  ...
  const handleClick =(value)=>{
    navigate('/projects/myProject',{ state: {ProjectId: value ,roleID:'Guide'} })
  }
  console.log(datas);
  return (
    <Layouts>
        <div className="flex w-full justify-center mt-5 ">
            <h1 className="text-2xl font-monospace font-bold">Projects Under {JSON.parse(temp).data[0].Name}</h1>
        </div>
