...
function Login() {
  const {userRole ,setUserRole,userID,setUserData} = useLoginManager();
  ...
  const userData = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/user?userId=${user.userId}&role=${user.role}`);
