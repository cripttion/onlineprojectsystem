...
const GuidesComponent = ({ projectID, token }) => {
 ...
 useEffect(()=>{
  ...
  const fetchGuides = async () => {
    try {
      const response = await axios.get(API_ENDPOINTS.GETGUIDEBYPROJECTID.replace("{projectID}", projectID), {
  ...
  const fetchAvailableTeachers = async () => {
    try {
      const response = await axios.get(API_ENDPOINTS.GETGUIDEALLOCATEDDATA, {
        headers: { 'x-access-token': token }
      });
      if (response.status === 200) {
  ...
  const handleAddGuide = async () => {
    ...
    try {
      await axios.post(
        API_ENDPOINTS.ADDNEWGUIDETOPROJECT, 
