import { AlignRight, ChevronDown, ChevronUp, LogOut, Settings, StepBackIcon, User, UserCog2 } from 'lucide-react'
import React, { useState } from 'react'
import { useNavigate } from 'react-router-dom';
import { useLoginManager } from '../StateManagement/UserManagement'
import Select from "react-select";
function Topnav({openData,setOpenData}) {
  ...
  return (
    ...
    <div className='flex flex-row md:inline-flex lg:inline-flex justify-between bg-bgBlue border-b border-white p-2 text-textColor1 items-center w-full'>
    {!openData&&<>
      <div onClick={handleOpenClick} className='cursor-pointer'><AlignRight/></div>
    </>}
